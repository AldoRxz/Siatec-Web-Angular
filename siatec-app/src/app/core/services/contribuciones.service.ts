...
export class ContribucionesService extends BaseApiService {
  ...
  getContribuciones(): Observable<any[]> {
    const url = this.buildUrl(this.baseUrl, 'contribuciones');
   ...
   * Usa POST determinaciones/calcular para crear y calcular la determinación.
   */
  calcularDeterminacion(payload: DeterminacionCalculoRequest): Observable<DeterminacionCalculoResponse> {
    const url = this.buildUrl(this.baseUrl, 'determinaciones', 'calcular');
    const body = {
